...
router.get('/stats', authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
        ...
        const all = await prisma.schedule.findMany({ select: { shift: true, status: true, date: true } })
        ...
        for (const s of all) {
            ...
            byStatus[s.status || 'scheduled'] = (byStatus[s.status || 'scheduled'] || 0) + 1
            if (s.date >= today && s.date < tomorrow) todayCount++
        }
        res.json({ success: true, data: { total: all.length, todayCount, byShift, byStatus } })
